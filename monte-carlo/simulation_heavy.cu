...
__device__ float Td[8];      // Time to maturity
__device__ float Kd[9];      // Strike price
__device__ float kappad[10]; // Kappa
__device__ float thetad[10]; // Theta
__device__ float sigmad[20]; // Sigma 
    ...
    int *valid_paths)
{
    ...
    param_idx -= kappa_idx * (10 * 20);
    int theta_idx = param_idx / 20; // sigma
    param_idx -= theta_idx * 20;
...
int main()
{
    ...
    float T[8] = {0.2f, 0.3f, 0.4f, 0.5f, 1.0f, 2.0f, 3.0f, 4.0f};
    float K[9] = {0.8f, 0.85f, 0.9f, 0.95f, 1.0f, 1.05f, 1.1f, 1.15f, 1.2f};

    // Admissible parameters provided by the professor:
    float sigma[20] = {0.1f, 0.12f, 0.13f, 0.14f, 0.15f, 0.16f, 0.17f, 0.18f, 0.19f, 0.2f, 0.21f, 0.22f, 0.23f, 0.24f, 0.25f, 0.26f, 0.27f, 0.28f, 0.29f, 0.30f};
    ...
    std::ofstream execution_time("data/execution_time_heavy.md");
    ...
    std::ofstream fout("data/vg_prices_all_heavy.csv");
    ...
    for (int t_idx = 0; t_idx < 8; t_idx++)
    {
        for (int k_idx = 0; k_idx < 9; k_idx++)
        {
            for (int kappa_idx = 0; kappa_idx < 10; kappa_idx++)
            {
                ...
                {
                    for (int sigma_idx = 0; sigma_idx < 20; sigma_idx++)
                    {
                        int idx = t_idx * (9 * 10 * 10 * 20) + // T * K * kappa * theta * sigma
                                  k_idx * (10 * 10 * 20) +     // kappa * theta * sigma
                                  kappa_idx * (10 * 20) +      // theta * sigma
                                  theta_idx * 20 +             // sigma
                                  sigma_idx;
    ...
    fout.close();
    std::cout << "Simulation completed. Results saved to vg_prices_all_heavy.csv" << std::endl;
